...
#
#
# def getDataLoader(dirname, percentage=0.7, shuffle=True, batch_size=64):
...
import random
from torch.utils.data import DataLoader
from myClasses import classesNameKey
import os
...
def getDataLoader(dirname, percentage=0.7, shuffle=True, batch_size=64):
    ...
    for i, label in enumerate(classes):
        ...
        temp = []
        for image_name in os.listdir(classes_path):
            temp.append((classes_path + '/' + image_name, classesNameKey[label]))
        boundary = int(len(temp) * percentage)
        random.shuffle(temp)
        for value in temp[:boundary]:
            # print(value)
